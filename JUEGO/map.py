...
class TacticalMap(Entity):
    ...
    def __init__(self, player, **kwargs):
        ...
        self.container = Entity(parent=self, enabled=False, position=(-0.2, 0))
        ...
        self.mission_panel = Entity(parent=self.container, model='quad', color=color.hex('#080d1a'), alpha=0.95, scale=(0.45, 0.65), position=(0.75, 0.05), z=0)
        ...
        if self.is_open:
            ...
            if hasattr(self.player, 'mission_manager'):
                ...
                for m in self.player.mission_manager.missions:
                    ...
                    card = Entity(parent=self.mission_panel, model='quad', color=color.hex('#0d1424'), alpha=0.9, scale=(0.95, 0.22), position=(0, y_offset - 0.05), z=-0.03)
                    
                    if getattr(m, 'is_main', False):
                        tipo_color = "<yellow>"
                        tipo_texto = "[PRINCIPAL]"
                        # Borde resaltado para principales
                        Entity(parent=card, model='quad', color=color.yellow, scale=(1.01, 1.05), z=0.01, alpha=0.5)
                    else:
                        tipo_color = "<orange>"
                    ...
                    # Titulo
                    t_title = Text(parent=self.mission_panel, text=f"{status_icon}{tipo_color}{tipo_texto}<default> {m.title}", color=status_color, scale=1.1, position=(-0.44, y_offset), z=-0.05)
                    # Descripcion
                    t_desc = Text(parent=self.mission_panel, text=m.description, color=color.light_gray, scale=0.85, position=(-0.44, y_offset - 0.06), z=-0.05, wordwrap=55)
                    
                    self.mission_texts.extend([card, t_title, t_desc])
                    
                    if m.completed:
                        Entity(parent=card, model='quad', color=color.rgba(0,0,0,180), scale=(1,1), z=-0.06)
